...
export const useFileUpload = (
  ...
  // Add a useEffect to handle file changes
  useEffect(() => {
  ...
  const clearFiles = useCallback(() => {
    setState((prev) => {
      ...
      // onFilesChange?.(newState.files) // REMOVE THIS LINE
      return newState;
    });
  }, []); // REMOVE onFilesChange from dependency array
    ...
    (newFiles: FileList | File[]) => {
      ...
      if (validFiles.length > 0) {
        ...
        setState((prev) => {
          ...
          // onFilesChange?.(newFiles) // REMOVE THIS LINE
          return {
            ...prev,
            files: newFiles,
            errors,
